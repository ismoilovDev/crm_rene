      ...
      <>
         ...
         <PdfWrapper indicator={documentInfo}>
               ...
               documentInfo?.data?.supply_infos?.[0]?.type === 'auto' || documentInfo?.data?.supply_infos?.[0]?.type === 'gold' ?
                  <>
                     ...
                     <div className='pdf_margin_top_20'>
                        <p className='distance'>Biz, quyida imzo chekuvchilar, {documentInfo?.data?.branch?.name} Kredit Komissiyasi raisi {fullName(documentInfo?.data?.branch?.head_of_branch)}, Kredit Komissiyasining a'zolari {fullName(documentInfo?.data?.branch?.chief_accountant)} ,{fullName(documentInfo?.data?.branch?.head_of_credit)} va {dateConvert(documentInfo?.contract_issue_date)} yil dagi {documentInfo?.contract_num}-sonli
            ...
            <div className='pdf_image_container'>
               {
                  documentInfo?.data?.supply_infos?.[0]?.images?.map((image, index)=>{
                     return(
                        <div key={index}>
                           <img
